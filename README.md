...
```bash
pip install -e ".[build]"
pyinstaller --onefile --windowed --name lgus-dat-desktop-V0.28.0 --distpath dist/desktop --workpath build/desktop --noconfirm src/lgus_dat/desktop/app.py
```

Replace `V0.28.0` with the current version from `pyproject.toml`.

The output will be in `dist/desktop/lgus-dat-desktop-V0.28.0.exe` (Windows) or `dist/desktop/lgus-dat-desktop-V0.28.0` (Linux).
